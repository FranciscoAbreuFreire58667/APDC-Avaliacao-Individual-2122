		...
		try {
			...
			if (check != null) {
				...
				Entity user = Entity.newBuilder(userKey).set("username", data.username).set("email", data.email)
						...
						.set("placeOfAddress", placeOfAddress).set("NIF", NIF)
						.set("profilePublic", profilePublic).set("role", NEW_UserRole.USER.getRole())
						.set("state", NEW_AccountState.INACTIVE.getValue()).build();
				txt.add(user);
				LOG.info("User registered " + data.username);
				txt.commit();
				return Response.ok("Registered with sucess.").build();
...
//			}
	...
	public Response changeAttrib(@PathParam("username") String username, NEW_UserChangeDataForUser data) {
		...
		try {
			...
			String role = user.getString("role");
			if (!possibleToChange(username, role, data.username, roleToBeChanged)) {
				txt.rollback();
				LOG.warning("Username: " + username + " does not permission to complete this action.");
				return Response.status(Status.BAD_REQUEST).entity("no permission.").build();
			} else {

				if (user == userToBeChanged && role.equals(NEW_UserRole.USER.getRole())) {
					String landlineNumber = data.landlineNumber;
					if (landlineNumber == null) {
	...
	public Response listUsers(@PathParam("username") String username) {
		...
		if (role.equals(NEW_UserRole.SU.getRole())) {
			...
			Query<Entity> queryUSER = Query.newEntityQueryBuilder().setKind("User")
					.setFilter(CompositeFilter.and(PropertyFilter.ge("role", NEW_UserRole.USER.getRole()),
	...
	private boolean possibleToRemove(String username, String role, String usernameToBeRemoved, String roleToRemove) {
		...
		if (role.equals(NEW_UserRole.USER.getRole()) && username != usernameToBeRemoved)
			return false;
	...
	private boolean possibleToChange(String username, String role, String usernameToBeChanged, String roleToBeChanged) {

		if (role.equals(NEW_UserRole.USER.getRole()) && username != usernameToBeChanged)
			return false;

		if (role.equals(NEW_UserRole.SU.getRole()) && role.equals(usernameToBeChanged)) {
			return false;
		}
		if (role.equals(NEW_UserRole.GS.getRole())) {
